    ...
    def step(self, eval):
        ...
        for i, (x_, y_) in enumerate(pbar := tqdm(loader, total=len(loader))):
            ...
            for bg_idx in range(n_mb):
                ...
                context_idxs, target_idxs = split_context_target(x, y, self.config['model']['context_percentage_low'], self.config['model']['context_percentage_high'])
                x_context = x[:, context_idxs, :].reshape(-1, 1, x_context.shape[-1]).to(device)
                y_context = y[:, context_idxs].reshape(-1, 1, y_context.shape[-1]).to(device)
                x_target = x[:, target_idxs, :].reshape(-1, 1, x_context.shape[-1]).to(device)
                y_target = y[:, target_idxs].reshape(-1, 1, y_context.shape[-1]).to(device)
                l2_truth = y
