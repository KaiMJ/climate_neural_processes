...
class Supervisor():
    ...
    def init_model(self):
        ...
        ray_dir = os.path.join(self.config["log_dir"], "ray")
        ray_config = {
    ...
    def step(self, eval):
        ...
        for i, (x, y, path) in enumerate(pbar := tqdm(loader, total=len(loader))):
            ...
            with torch.cuda.amp.autocast():
                ...
                if torch.sum(torch.isnan(l2_output_mu)) > 0:
                    print(path)
                    print(' ')
            ...
            if eval is not False:
                ...
                writer.add_scalar("norm_rmse", norm_rmse, self.global_batch_idx)
